...
class window_tk():
    ...
    def activate_video(self):
        ...
        self.vid = cv2.VideoCapture(1)#r"D:\WORK\allpy\venv\first_bot\aruco_video.mp4")
    ...
    def show_frames(self):
        ...
        if len(corners) != 0:
            ...
            if(len(self.destination)>0):
                ...
                if len(self.path) > 0:
                    ...
                    if(self.tempId< len(self.path)):
                        if round(math.dist(self.getMarkerCenter(self.corners[self.indexId[1]][0]), self.path[self.tempId][0])) >= 20:
        ...
        self.canvas.after(200,self.show_frames)
    ...
    def detect_aruco(self,frame):
        ...
        return corners,ids
    def anglerotated(self,corners,dest,center_vec):
        print("inside it")
        unit_x_axis = [1.,0.]
        dest = np.array(dest)
        ...
        dest_prod = np.dot(unit_vec,center_vec)
        dest_angle = np.arccos(dest_prod)
    ...
    def odomentaryData(self,corners,bot,botID):
        if len(bot)>1: # only calling this function if bot destination is assigned 
            ...
            center_unit_vec = (np.array(bot)-np.array(center))/np.linalg.norm(np.array(bot)-np.array(center))
            theta,crcted_angle = self.anglerotated(corners,bot,center_unit_vec)
            print("angle created",theta,crcted_angle)
    ...
    def botSteer(self,botId):
        ...
        if(Dc >= 15):
            if dist_theta > 140:
                ...
                print(cmd)
                self.sendCommands(cmd)
            elif(Dl>Dr and abs(Dl-Dr)>20): #right
                # angle = abs(theta - dist_theta)
                # print(angle)
                # ticks = angle%15
                ticks = 180 #int(ticks*50)
                #cmd = "<4 "+str(ticks)+" 0 0>"
                cmd = "<0 0 4 " + str(ticks) + ">"
                print(cmd)
                self.sendCommands(cmd)
